...
以下配方在實際專案跑過一輪，可直接沿用。**動手前一定要先問使用者**——這個決定會讓專案從「純靜態、雙擊打開就能看」變成「要跑過編譯指令才有最新樣式」，是專案性質的改變，不是純技術選擇。
...
### 實跑補充：套件替換對照（JSnews5.3.7 專案驗證過）

以下是實際跑過一輪節點 9 才確認的具體對照，記下來是因為「查是否停止維護」跟「找到替代品」兩步都要花時間重新查證，先列出已驗證過的結果可以省下下一個專案重複查證的時間：

| 舊套件 | 判定 | 替代方案 | 陷阱 |
|---|---|---|---|
| FlexSlider | 已停止維護 | Swiper | 換函式庫後，舊函式庫專屬的 CSS（例如 `.flexslider .slides>li{display:none}`）可能跟新函式庫的預設顯示機制衝突，導致整塊空白；只看 scrollHeight／溢出量兩個不變量測不出來，要看截圖或量子元素的 bounding box 才抓得到 |
| Magnific Popup／prettyPhoto／fancyBox | 三者都已停止維護或功能重疊 | GLightbox（單一套件可同時取代三者） | 動手前先逐一確認每個套件是否真的有初始化呼叫端——舊站常見「載入了套件的 CSS/JS，但從未真正呼叫初始化函式」的孤兒套件，這種直接移除即可，不需要找替代品 |
| Isotope | 官方新版已改為商用授權，不是升版號能解決的問題 | 視實際排版需求改用原生 CSS（`display:flex;flex-wrap:wrap` 或 `display:grid`），不是找替代套件 | 若原本欄寬用的是 Bootstrap 格線 class（`col-*`），flex-wrap 通常可以讓子元素原本的 width% 直接生效，不需要重寫斷點；grid 則需要重新設計欄寬 |
| Modernizr | 官方仍在維護，但多數舊站用它做的「瀏覽器特性偵測」現在已是所有主流瀏覽器原生支援 | 若全站 CSS 沒有依賴 `.js`／`.no-js` class、JS 也沒有查詢 `Modernizr.xxx`，可直接移除，不需要升版 | 移除前務必先 grep 確認 CSS 與 JS 兩邊都零依賴，不能只看其中一邊 |
| MediaElement.js | 官方仍在維護 | 若確認有真的被使用，可直接升版；若從未真正初始化過，直接移除 CDN 載入 | 舊站常見「HTML 有觸發 class，但初始化用的 JS 從未真正掛上該 class」的孤兒套件，跟上面的燈箱套件是同一類陷阱，動手前先確認全站有沒有真的呼叫初始化 |

**共通判斷順序**：先確認套件在全站有沒有真的被初始化呼叫（不是只看 HTML/CSS 有沒有殘留 class，是看 JS 有沒有真的執行到初始化那一行），零呼叫端的直接移除，不用找替代品；有呼叫端才進入「查官方維護狀態→找替代品」的標準流程。
