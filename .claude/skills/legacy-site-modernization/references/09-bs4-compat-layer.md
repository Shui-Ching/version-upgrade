...
[`assets/bs4-compat.css`](../assets/bs4-compat.css) 是起手樣板，涵蓋 Reboot、Grid、表單、
卡片、關閉鈕、方向性 utility 六組。檔案開頭自述數值取自 BS 4.6.2 原始碼——
**但不要把這句當成可以省略核對的理由**，實際該用的值以下面第 5 點為準。
複製到專案的 `css/` 之後**用刪的，不要用加的**：

1. 依盤點結果**刪掉站上沒用到的段落**。
2. 標【專案專屬】的段落必須改寫或刪除。目前樣板裡只有一段：
   `.modal { --bs-modal-color: inherit }`，用途是還原 BS4「字色從外層繼承」的行為。
   保持 `inherit` 就是還原框架舊行為；若照註解換成專案的字色 token，
   請確認那個 token 的值確實等於升級前實際繼承到的顏色，否則就變成藉相容層改設計，
   違反下面那條判準。
3. 最下方的方向性 utility 是完整對照表，只留盤點有命中的；間距類的階數與斷點變體要自行展開
   （樣板只放了 `.ml-2`／`.mr-2` 與一個 `@media` 斷點變體當示範）。
4. 補上稽核腳本抓到、樣板未涵蓋的風險點。
5. **逐條把數值換成從 `/tmp/bs4.css` 讀出來的實際值**——樣板寫的是 4.6.2 的框架預設值，
   專案若載入過客製主題檔（BS3 站的 `bootstrap-theme.css` 是典型），實際生效的值會不一樣。
   這個陷阱在 [04 的「要調回哪些值」](04-bootstrap-upgrade.md#要調回哪些值從專案自己的舊檔案讀不要背表) 有實例。

**有五段的註解寫了「什麼情況下不需要補」，刪之前先讀**：`:root { scroll-behavior }`（只在有 JS
捲動動畫時需要）、`.row > *` 的 `position`（只在欄位內有絕對定位元素時需要）、
`.form-control` 高度（只在自家 CSS 動過表單 padding 時需要）、`.card-body` padding
（只在站上有「`.card-body` 沒有 `.card` 祖先」的結構時需要）、方向性 utility（只留盤點命中的）。
**其餘段落只寫了「BS5 移除了什麼」，沒寫什麼時候可以不補**——那幾段要自己回頭對盤點結果，
不要因為註解沒叫你刪就留著。
