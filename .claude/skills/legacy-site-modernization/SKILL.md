...
把「接手一個舊版純靜態網站，要整理到能放心維護、能通過資安檢測」這件事拆成 11 個節點。前 10 個彼此有嚴格的順序依賴，這套流程是從 `tpl_fortune_tcnews` 專案實際跑出來的（節點 1、2 已有對應 commit：`d2d49b2`、`5304016`；節點 5「CSS 轉 SCSS + 導入 npm/sass 建置工具」對應 commit `75cb025`），下一個類似的舊專案可以直接照這份清單走。節點 11（加上不支援 IE 提示）只依賴節點 7，不吃前 10 個節點的順序鏈，排在清單最後純粹是為了不動到既有編號。
...
`版升專案`（`C:\Users\andrew\Desktop\版升專案\.claude\skills\legacy-site-modernization\`）是這份 skill 的**權威版本（single source of truth）**。JSnews5.3.7、TCnews5.3.7 等其他專案底下的副本，是為了在地執行方便才放的複本，不是各自獨立維護的版本。

**回寫規則**：只要在任何專案執行這份 skill 的過程中，發現新的陷阱、修正了既有描述的錯誤、補齊了某個節點的做法、或新增了腳本／樣板，動作完成後要把「有價值、下一個舊站也用得到」的那部分同步寫回 版升專案 的副本——不是整包覆蓋對方，是把這次新學到、可重複使用的內容搬過去。版升專案 底下有 git，寫回後順手 commit，訊息開頭比照既有慣例（`merge: ...` 或 `fix: ...`），方便之後追溯是哪個專案的經驗貢獻回來的。

**什麼算「有價值」、什麼不算**：
- **算**：新發現的 Bootstrap／jQuery 行為陷阱、修正過的項目數量或判準、新的稽核腳本、通用的第三方套件替換對照（例如「FlexSlider 已停止維護，換 Swiper」這種跨專案都適用的結論）。
- **不算**：這個專案特有的檔名、commit hash、專案自己的業務邏輯或美術規格。這些留在專案自己的副本或該次的 commit message 就好，不必寫進 版升專案，寫進去只會讓權威版本混進單一專案的雜訊。

**其他專案的副本什麼時候要跟著更新**：不必每次執行都主動比對——版升專案 更新後不會自動推播到其他副本。開始在某個專案跑這份 skill 之前，建議先跟 版升專案 的副本做一次 diff，確認沒有漏接其他專案回寫進去的新內容；發現對方明顯更新，且自己這份沒有尚未回寫的在地修改時，直接整包覆蓋最省事——但覆蓋前務必先確認自己這份沒有還沒寫回去的東西，否則會被蓋掉。
